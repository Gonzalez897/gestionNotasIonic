<template>
        ...
        <ion-list>
          ...
          <ion-item><br>
            ...
            <ion-select v-model="materia.idProfesores">
              <ion-select-option v-for="(x,i) in profesor" :key="i" :value="x.idProfesores">
              {{ x.nombreProfesor }}
              </ion-select-option>
            </ion-select>
          </ion-item>
          <ion-button expand="block" @click="guardarFormulario()">Guardar</ion-button>
        </ion-list>
      </ion-content>
    </ion-page>
  </template>
  ...
  <script>
  import { IonPage, IonHeader, IonToolbar, IonList, IonContent, IonLabel, IonInput, IonItem, IonButton, IonTitle, IonSelect} from '@ionic/vue';
import axios from 'axios';
  ...
  export default {
    name: 'MateriaView',
    components:{
        IonPage, IonHeader, IonToolbar, IonList, IonContent, IonLabel, IonInput, IonItem, IonButton, IonTitle, IonSelect },
    data() {
      return {
    ...
    methods: {
      guardarFormulario() {
        ...
        axios.post('http://127.0.0.1:8000/api/Materia/store', this.materia)
        .then(response=>{
          alert("se ha ingresado la materia") 
          this.materia.nombreMateria=''
          this.materia.idProfesor=''
        })
        .catch(error=>console.log("Hubo un error al tratar de ingresar la materia "+error))
      ...
      guardarDocente(){
        axios.get('http://127.0.0.1:8000/api/profesores/select')
        .then(response=>{
